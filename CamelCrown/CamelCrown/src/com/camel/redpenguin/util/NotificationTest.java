...
public class NotificationTest extends Activity {
	...
	private void init() {
		...
		pd = PendingIntent.getActivity(NotificationTest.this, 0, intent, 0);
	}
	...
	OnClickListener onclick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.le10bt01:
				...
				// 第四个参数：点击该通知时执行页面跳转
				baseNF.setLatestEventInfo(mContext, "Title01", "Content01", pd);
				// 发出状态栏通知
			...
			case R.id.le10bt02:
				...
				baseNF.setLatestEventInfo(mContext, "Title02", "Content02", pd);
				nm.notify(Notification_ID_BASE, baseNF);
				break;
			...
			case R.id.le10bt04:
				...
				mediaNF.setLatestEventInfo(mContext, "Title03", "Content03",
						pd);
            ...
            /**  
				...
				// 第三个参数：下拉状态栏时显示的消息内容 expanded message text
				// 第四个参数：点击该通知时执行页面跳转
				// baseNF.setLatestEventInfo(Text.this, "Title01", "Content01",
				// pd);
				...
				nm.notify(Notification_ID_BASE, baseNF);*/
				/**
				 * //自定义下拉视图，比如下载软件时，显示的进度条。 Notification notification = new
				 * Notification();
				 * 
				 ...
				 * R.layout.text2); contentView.setImageViewResource(R.id.image,
				 * R.drawable.ic_launcher);
				 * contentView.setTextViewText(R.id.text,
				 * "Hello, this message is in a custom expanded view");
				 * notification.contentView = contentView;
				 * 
				 * //使用自定义下拉视图时，不需要再调用setLatestEventInfo()方法 //但是必须定义
				 * contentIntent notification.contentIntent = pd; // 3
